...
public enum WildflowerPlantingMesh {
    ...
    public enum Kind: Int, CaseIterable, Sendable { case tuft, poppy, lupine, goldfields, popcorn }
    ...
    public static func weights(at p: Vec2, seed: UInt64) -> [(Kind, Double)] {
        weights(drift: valueNoise(p * 0.22, seed: seed), valueNoise(p * 0.31 + Vec2(40, 17), seed: seed &+ 7))
    }
    ...
    public static func plants(polygon: [Vec2], density: Double, seed: UInt64) -> [Plant] {
        ...
        let perSquareMeter = min(wanted, Double(maxTriangles) / max(1, meanTrianglesPerPlant) / area)
        ...
        for iy in Int((y0 / cell).rounded(.down)) ... Int((y1 / cell).rounded(.up)) {
            for ix in Int((x0 / cell).rounded(.down)) ... Int((x1 / cell).rounded(.up)) {
                ...
                let table = weights(at: p, seed: seed)
    ...
    public static func layers(polygon: [Vec2], density: Double = 1,
                              seed: UInt64 = 1, groundHeight: (Vec2) -> Double) -> [Layer] {
        var out: [Layer] = []
        for plant in plants(polygon: polygon, density: density, seed: seed) {
