...
public enum UnitBond: String, Equatable, Hashable, Sendable, Codable, CaseIterable {
    ...
    case thirdRunning
    /// **Herringbone** — interlocking 2:1 bricks laid in a diagonal zigzag. This is NOT a course
    /// offset: it is its own lattice (a horizontal brick's end meets the next brick's side), so it
    /// is generated by `MaterialGenerator.tileLattice` rather than the offset path below. The block
    /// repeats every `2L` short-edges on both axes (`L` = brick length in short-edges, here 2), so a
    /// bake sized to a multiple of `2L` wraps by construction — the seamless math DH-0099 asked for.
    case herringbone
    /// **45° diagonal** — a diamond grid, the square lattice rotated a quarter-turn onto its point.
    /// A literal 45° rotation changes a square's period by √2 and stops wrapping (why it used to be
    /// refused); instead the diamond lattice `(a,b) = (u+v, u−v)` is sampled directly, so at each
    /// bake seam `a` and `b` step by whole cells and the pattern wraps for any integer cell count.
    case diagonal

    /// True for the two arrangements that are their own lattice rather than a course offset — they
    /// bypass `offset(row:)`/`rowCycle` and bake through `MaterialGenerator.tileLattice`.
    public var isLattice: Bool { self == .herringbone || self == .diagonal }
    ...
    /// The lattice arrangements carry no course cycle (their wrap is handled in `TileLayout`), so
    /// they report 1.
    public var rowCycle: Int {
        ...
        case .herringbone:  return 1
        case .diagonal:     return 1
        }
    }

    /// The u-shift (in units) applied to course `row`. Zero for the lattice arrangements — they do
    /// not lay in courses, so no per-row shift applies.
    public func offset(row: Int) -> Double {
        switch self {
        case .stack:        return 0
        case .running:      return (((row % 2) + 2) % 2 == 0) ? 0 : 0.5
        case .thirdRunning: return Double(((row % 3) + 3) % 3) / 3.0
        case .herringbone:  return 0
        case .diagonal:     return 0
        }
    }
    ...
    public var displayName: String {
        ...
        case .diagonal:     return "Diagonal 45°"
        }
    }
}
...
public struct TileLayout: Equatable, Hashable, Sendable {
    ...
    /// Herringbone brick length in short-edges. Classic herringbone is 2:1, and a 1:1 "brick" has
    /// no zigzag, so the arrangement forces `L = 2` regardless of the chosen unit shape.
    public static let herringboneBrickLength = 2
    ...
    public static func of(_ params: UnitPatternParams, on tiling: SurfaceTiling,
        ...
        switch params.bond {
        case .herringbone: return herringbone(params, on: tiling, bakeSize: bakeSize)
        case .diagonal:    return diagonal(params, on: tiling, bakeSize: bakeSize)
        default:           break
        }
    ...
    /// **Herringbone layout.** `cols` counts SHORT-edges across the bake (the brick is `2L`
    /// short-edges' block, laid on both axes), and it is snapped to a multiple of `2L` so the block
    /// repeats a whole number of times — the wrap condition the diagonal-zigzag pattern needs. The
    /// user's `unitSizeMeters` is the brick's LONG edge, so one short-edge is `unitSize / L`.
    private static func herringbone(_ p: UnitPatternParams, on tiling: SurfaceTiling,
                                    bakeSize: Int) -> TileLayout {
        let L = herringboneBrickLength
        let block = 2 * L
        let shortEdge = Swift.max(p.unitSizeMeters / Double(L), 1e-4)
        let target = tiling.isAdjustable ? Swift.min(tiling.metresPerRepeat, targetRunMeters)
                                         : tiling.metresPerRepeat
        let cap = Swift.max(block, (bakeSize / minShortEdgeTexels / block) * block)
        var cells = Int((target / shortEdge / Double(block)).rounded()) * block
        cells = Swift.max(block, Swift.min(cells, cap))
        let run = tiling.isAdjustable ? Double(cells) * shortEdge : tiling.metresPerRepeat
        return TileLayout(cols: cells, rows: cells, runMeters: run, bakeSize: bakeSize)
    }

    /// **45° diagonal (diamond) layout.** `cols` is the number of unit cells along each bake axis;
    /// any WHOLE count wraps, because the rotated lattice `(u+v, u−v)` steps by whole cells at each
    /// seam. `unitSizeMeters` is the tile's edge; the diamond's on-point spacing follows from it.
    private static func diagonal(_ p: UnitPatternParams, on tiling: SurfaceTiling,
                                 bakeSize: Int) -> TileLayout {
        let cap = Swift.max(2, bakeSize / minShortEdgeTexels)
        let target = tiling.isAdjustable ? Swift.min(tiling.metresPerRepeat, targetRunMeters)
                                         : tiling.metresPerRepeat
        var cells = Int((target / Swift.max(p.unitSizeMeters, 1e-4)).rounded())
        cells = Swift.max(2, Swift.min(cells, cap))
        let run = tiling.isAdjustable ? Double(cells) * p.unitSizeMeters : tiling.metresPerRepeat
        return TileLayout(cols: cells, rows: cells, runMeters: run, bakeSize: bakeSize)
    }
}
...
public extension MaterialGenerator {
    ...
    static func tile(size: Int = MaterialGenerator.bakeSize,
        ...
        // Herringbone and 45° diagonal are their own lattices, not course offsets — the offset-grid
        // loop below cannot express them. They bake through `tileLattice`, which is seamless by the
        // same wrap arithmetic (`TileLayout.herringbone`/`.diagonal` sized the grid to close). DH-0099.
        if p.bond.isLattice {
            return tileLattice(size: size, params: params, tiling: tiling,
                               layout: layout, faceImage: faceImage)
        }
    ...
    /// **The lattice tile bakes — herringbone and 45° diagonal (DH-0099).** These two arrangements
    /// are not course offsets over a rectangular grid (`tile`'s loop), they are their own lattices:
    ///
    /// * **Herringbone** lays 2:1 bricks in a diagonal zigzag. The brick covering short-cell `(i,j)`
    ///   is found analytically: `s = (i+j) mod 2L` selects orientation (`s < L` ⇒ horizontal, the
    ///   brick's left `L` cells; else vertical, the bottom `L` cells). That assignment has period
    ///   `2L` on both axes, so a bake whose cell count is a multiple of `2L` (`TileLayout.herringbone`
    ///   guarantees it) wraps — the pattern, its joints and its per-brick tone are all identical
    ///   across each seam.
    /// * **45° diagonal** samples the rotated lattice `(a,b) = (u·N + v·N, u·N − v·N)` directly. A
    ///   literal rotation stretches the period by √2 and stops wrapping; sampling `(a,b)` instead
    ///   steps `a` and `b` by exactly `N` whole cells at each bake seam, so the diamonds wrap for any
    ///   integer `N`. This is the seam-free math the old `UnitRun` note flagged as a follow-up.
    ///
    /// Shading (per-unit tone, glaze pooling, warp/macro/micro, the recessed joint layer, imported
    /// face image, bevel) is the SAME vocabulary `tile` uses, so a herringbone tile reads as the
    /// same material as a stacked one — only the lattice differs.
    static func tileLattice(size: Int, params: TileParams, tiling: SurfaceTiling,
                            layout: TileLayout, faceImage: MaterialChannels?) -> MaterialChannels {
        let p = params.pattern
        let cells = Swift.max(1, layout.cols)          // short-cells (herringbone) / unit cells (diag)
        var ch = MaterialChannels(size: size, category: .tile)
        // A lattice tile carries its per-unit variation baked in (it repeats every block), and the
        // engine's `patternCells` grid de-repeat cannot key a diagonal/zigzag lattice — a square
        // grid hash would cut a tone step through the middle of a brick. So it opts out, like a
        // bonded axis does, rather than declaring a lattice it does not have.
        ch.patternCells = .zero
        ch.patternJitter = 0
        ...
        // Joint half-width and antialias band, in the lattice's own cell units, floored to the
        // thinnest line the bake can hold (below `minJointTexels` the grout aliases and the wrap
        // stops matching — the same floor `tile` takes from `TileLayout`).
        let texelsPerCell = Double(size) / Double(cells)
        let soft = Swift.max((herring ? 1.0 : 1.4142) / Swift.max(texelsPerCell, 1), 0.004)
        let jf = p.jointFraction
        // Herringbone: joint is `jf` of the LONG (L-cell) edge, measured in cells. Diagonal: `jf` of
        // the diamond edge, in `a`/`b` units, where one cell of `a` spans edge/√2 → ×√2.
        let jointHalf = herring ? jf * Double(L) / 2 : jf * 1.4142 / 2
        let jointHalfFloored = Swift.max(jointHalf, TileLayout.minJointTexels / (2 * Swift.max(texelsPerCell, 1)))

        for y in 0..<size {
            for x in 0..<size {
                // A vertical run is a quarter turn of the lattice — swap the sampling axes, exactly
                // as `tile` does, so a vertical herringbone is seamless the same way.
                let su = (Double(x) + 0.5) / Double(size)
                let sv = (Double(y) + 0.5) / Double(size)
                let u = p.run == .vertical ? sv : su
                let v = p.run == .vertical ? su : sv

                // Per-texel lattice classification → the SAME shading inputs both bakes share:
                //   dMin  = distance to the nearest joint, in cell units
                //   keyI/keyJ = wrapped per-unit id for the tone hash (identical across the seam)
                //   faceU/faceV = within-unit coordinate for an imported face image
                var dMin = 0.0, faceU = 0.0, faceV = 0.0, keyI = 0, keyJ = 0
                if herring {
                    let gx = u * Double(cells), gy = v * Double(cells)
                    let i = Int(floor(gx)), j = Int(floor(gy))
                    let s = (((i + j) % (2 * L)) + 2 * L) % (2 * L)
                    let ax: Int, ay: Int, lu: Int, lv: Int
                    if s < L { ax = i - s; ay = j; lu = L; lv = 1 }      // horizontal brick
                    else     { ax = i;     ay = j - (s - L); lu = 1; lv = L }  // vertical brick
                    let lx = gx - Double(ax), ly = gy - Double(ay)
                    let dU = Swift.min(lx, Double(lu) - lx)
                    let dV = Swift.min(ly, Double(lv) - ly)
                    dMin = Swift.min(dU, dV)
                    faceU = lx / Double(lu); faceV = ly / Double(lv)
                    keyI = ((ax % cells) + cells) % cells
                    // Fold orientation in so an H brick and a V brick sharing an anchor coordinate
                    // still draw as two different cuts.
                    keyJ = (((ay % cells) + cells) % cells) + (s < L ? 0 : cells)
                } else {
                    let a = (u + v) * Double(cells)
                    let b = (u - v) * Double(cells)
                    let fa = a - floor(a), fb = b - floor(b)
                    dMin = Swift.min(Swift.min(fa, 1 - fa), Swift.min(fb, 1 - fb))
                    faceU = fa; faceV = fb
                    keyI = ((Int(floor(a)) % cells) + cells) % cells
                    keyJ = ((Int(floor(b)) % cells) + cells) % cells
                }

                let joint = 1 - smoothstep(jointHalfFloored, jointHalfFloored + soft, dMin)

                // Per-unit discrete tone — the thing that stops a lattice reading as clones.
                let th = Noise.hash2(keyI, keyJ, p.seed)
                let tone = 1 + (Noise.unit(th) - 0.5) * p.toneVariation
                let warp = Noise.fbmTiled(u, v, baseCells: Swift.max(cells, 2) * 3, octaves: 3,
                                          seed: p.seed ^ 0x11)
                let macro = 0.97 + 0.06 * Noise.fbmTiled(u, v, baseCells: 2, octaves: 3,
                                                         seed: p.seed ^ 0x22)

                // Glaze pooling: thicker (darker, glossier) toward the unit edge, from the
                // normalised edge distance so it stays symmetric per unit and seamless.
                let edgeN = dMin / Swift.max(jointHalfFloored, 1e-6)
                let pool = 1 - smoothstep(1.0, 3.0, edgeN)

                var bodyCol: Vec3
                if let face {
                    let fu = Swift.min(Swift.max(faceU, 0), 0.999999)
                    let fv = Swift.min(Swift.max(faceV, 0), 0.999999)
                    let sx = Int(fu * Double(face.size)), sy = Int(fv * Double(face.size))
                    bodyCol = face.albedo[face.idx(sx, sy)] * tone
                } else {
                    bodyCol = p.bodyColor * tone * (0.96 + 0.08 * warp) * (1 - 0.09 * pool)
