...
final class IlluminatoramaFrameUniformsLayoutTests: XCTestCase {
    ...
    private static let metalStride = 1168
    ...
    func testTrailingClustersSitAtMetalOffsets() {
        assertOffset(\.nightSkyParams,  1072, "nightSkyParams")
        assertOffset(\.nightMoonDir,    1088, "nightMoonDir")
        assertOffset(\.nightSunDir,     1104, "nightSunDir")
        assertOffset(\.lensFlareParams, 1120, "lensFlareParams")
        assertOffset(\.halationParams,  1136, "halationParams")
        assertOffset(\.halationTint,    1152, "halationTint")
    }
