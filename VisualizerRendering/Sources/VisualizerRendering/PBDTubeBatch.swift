...
public final class PBDTubeBatch {
    ...
    public func tick(tubes: [PBDTubeRenderer], dt: Double, params: Params) {
        ...
        while accumulator >= fixedDt && steps < 4 {
            ...
            // Phase 4 — tube-mesh expand: writes the render mesh from the final
            // particle positions, in the SAME substep command buffer.
            // (A "once per tick, separate CB" optimisation was tried and REVERTED
            // 2026-06-21: that made the mesh UPDATE a separately-droppable command
            // buffer, so under GPU load the in-flight ring dropped the expand and
            // the franks' drawn position froze for a frame → visible frame-skip on
            // the way down. Keeping expand in the substep CB ties the mesh update
            // to the solve that produced it — no independent drop. ~1 ms of
            // redundant expand is well worth the smoothness.)
